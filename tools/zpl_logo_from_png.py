#!/usr/bin/env python3
"""Convierte un PNG local a comando ZPL ^GFA (ASCII HEX)."""
...
def rgba_to_bw_pixels(image: RawImage, threshold: int) -> list[int]:
    ...
    for r, g, b, a in image.pixels:
        ...
        bw.append(1 if gray < threshold else 0)
    return bw
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convertir PNG a ZPL GFA")
    parser.add_argument("--in", dest="input_path", required=True, help="Ruta del PNG de entrada")
    parser.add_argument("--out", dest="output_path", required=True, help="Ruta del TXT de salida")
    parser.add_argument("--width", type=int, default=150, help="Ancho final en dots")
    parser.add_argument("--threshold", type=int, default=128, help="Umbral 1-bit (0-255)")
...
def main() -> None:
    ...
    raw = parse_png(input_path.read_bytes())
    resized = resize_nearest(raw, args.width)
    bw = rgba_to_bw_pixels(resized, args.threshold)
