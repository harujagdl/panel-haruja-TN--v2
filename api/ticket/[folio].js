...
async function buildPdfBuffer({ folio, fecha, cliente, contacto, items, subtotal, anticipo, descuento, total }) {
  ...
  for (const item of items || []) {
    page.drawText(String(item.Codigo || ""), { x: 40, y, size: 9, font: bold });
    page.drawText(String(item.Descripcion || "Prenda").slice(0, 33), { x: 110, y, size: 9, font });
    page.drawText(`$${roundMoney(item.Precio).toFixed(2)}`, { x: 320, y, size: 9, font });
    y -= 14;
    if (y < 130) break;
  }
  ...
  y -= 18;
  page.drawText(`Total de cuenta: $${roundMoney(total).toFixed(2)}`, { x: 40, y, size: 12, font: bold });
