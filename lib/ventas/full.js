...
export async function getVentasFullData(monthValue = '', { forceRefresh = false, debug = false } = {}) {
  ...
  assignmentRows.forEach((row) => {
    ...
    latestAssignmentByOrder.set(assignment.order_id, assignment.seller);
  });
  ...
  const ventas = salesRows
    .map((row) => {
      ...
      const assignmentSeller = latestAssignmentByOrder.get(venta.order_id);
      if (assignmentSeller) {
        venta.seller = assignmentSeller;
