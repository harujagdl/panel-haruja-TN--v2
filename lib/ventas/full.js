...
function monthFromDate(value) {
  return getMexicoMonthKey(value) || '';
}
...
function mapVenta(headers = [], row = []) {
  ...
  const fechaOperativa = String(getValueByAliases(byKey, ['fecha_operativa']) || getMexicoDateKey(createdAt)).trim();
  ...
  return {
    ...
    month_key: String(getValueByAliases(byKey, ['month_key']) || monthFromDate(fechaOperativa || createdAt)).trim(),
