...
export async function getVentasFullData(monthValue = '', { forceRefresh = false, debug = false } = {}) {
  ...
  const ventasRange = `${SHEET_VENTAS}!A1:R1000`;
  ...
  console.log('[ventas-full] sheets_ventas_debug', {
    ...
    values_0: Array.isArray(ventasRowsRaw[0]) ? ventasRowsRaw[0] : [],
  });

  if (!ventasRowsRaw || ventasRowsRaw.length < 2) {
    throw new Error(`La hoja de ventas "${SHEET_VENTAS}" está vacía o no contiene filas de datos.`);
  }
  ...
  if (!headers.length || missingRequiredHeaders.length) {
    throw new Error(`La hoja de ventas "${SHEET_VENTAS}" está vacía o no contiene encabezados.`);
  }
