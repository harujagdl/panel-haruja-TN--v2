...
const SHEET_ASSIGNMENTS_LOG = 'VentasAsignacionesLog';
...
function parseItemsValue(rawValue) {
  if (!rawValue) return [];
...
function mapAssignment(headers = [], row = []) {
  const byKey = {};
  headers.forEach((header, index) => {
    const normalized = normalizeHeader(header);
    if (normalized) byKey[normalized] = row[index] ?? '';
  });
  const orderId = String(byKey.order_id || '').trim();
  const sellerRaw = String(byKey.seller_nuevo || byKey.seller || byKey.vendedora || '').trim().toLowerCase();
  const seller = sellerRaw === 'haru' ? 'Haru' : (sellerRaw === 'vendedora' ? 'Vendedora' : '');
  const timestamp = String(byKey.timestamp || byKey.updated_at || byKey.created_at || '').trim();
  if (!orderId || !seller) return null;
  return { order_id: orderId, seller, timestamp };
}
...
function mapVenta(headers = [], row = []) {
  ...
  const items = parseItemsValue(getValueByAliases(byKey, ['items', 'order_products', 'line_items', 'productos', 'detalle_productos']));
  ...
  return {
    ...
    fecha_operativa: String(getValueByAliases(byKey, ['fecha_operativa']) || getMexicoDateKey(createdAt)).trim(),
    month_key: String(getValueByAliases(byKey, ['month_key', 'monthkey']) || getMexicoMonthKey(createdAt)).trim(),
...
export async function getVentasFullData(monthValue = '', { forceRefresh = false, debug = false } = {}) {
  ...
  const assignmentRange = `${SHEET_ASSIGNMENTS_LOG}!A1:Z`;
  ...
  const [ventasResponse, statusResponse, assignmentsResponse] = await Promise.all([
  ...
  const assignmentRowsRaw = assignmentsResponse?.data?.values || [];
  ...
  const assignmentHeaders = Array.isArray(assignmentRowsRaw[0]) ? assignmentRowsRaw[0] : [];
  const assignmentRows = assignmentHeaders.length ? assignmentRowsRaw.slice(1) : [];
  const latestAssignmentByOrder = new Map();
  assignmentRows.forEach((row, index) => {
    const assignment = mapAssignment(assignmentHeaders, row);
    if (!assignment) return;
    const previous = latestAssignmentByOrder.get(assignment.order_id);
    const assignmentTs = Date.parse(assignment.timestamp);
    const previousTs = Date.parse(previous?.timestamp || '');
    const assignmentTsValid = Number.isFinite(assignmentTs);
    const previousTsValid = Number.isFinite(previousTs);
    if (!previous) {
      latestAssignmentByOrder.set(assignment.order_id, { ...assignment, _idx: index });
      return;
    }
    if (assignmentTsValid && previousTsValid) {
      if (assignmentTs >= previousTs) latestAssignmentByOrder.set(assignment.order_id, { ...assignment, _idx: index });
      return;
    }
    if (assignmentTsValid && !previousTsValid) {
      latestAssignmentByOrder.set(assignment.order_id, { ...assignment, _idx: index });
    ...
    if (!assignmentTsValid && !previousTsValid && index >= (previous?._idx ?? -1)) {
      latestAssignmentByOrder.set(assignment.order_id, { ...assignment, _idx: index });
    }
  });
  ...
  const ventas = salesRows
    .map((row) => {
      const venta = mapVenta(headers, row);
      const assignment = latestAssignmentByOrder.get(venta.order_id);
      if (assignment?.seller) {
        venta.seller = assignment.seller;
