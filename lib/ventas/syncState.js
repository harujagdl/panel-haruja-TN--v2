...
async function ensureSheetExists(sheets) {
  ...
  const exists = (metadata?.sheets || []).some((sheet) => normalize(sheet?.properties?.title) === normalize(SHEET_NAME));
  if (!exists) {
...
export async function readVentasSyncStateSafe() {
  const sheets = createSheetsClient({ readOnly: true });
  const metadata = await getSpreadsheetMetadata(sheets);
  const exists = (metadata?.sheets || []).some((sheet) => normalize(sheet?.properties?.title) === normalize(SHEET_NAME));
  if (!exists) {
    const empty = {};
    REQUIRED_KEYS.forEach((key) => { empty[key] = ''; });
    return empty;
  }
  const rows = await readSheetRowsRaw(sheets, `${SHEET_NAME}!A2:C`);
  const rowMap = new Map();
