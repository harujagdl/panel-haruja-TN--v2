...
export async function registerWebhookEventAttempt({
  ...
  const headers = await getSheetHeadersRaw(sheets, SHEET_NAME);
  const rows = await readSheetRowsRaw(sheets, `${SHEET_NAME}!A2:ZZ`);
  const records = rows.map((row) => rowToRecord(headers, row));
  const duplicated = records.some((record) => String(record.event_key || '').trim() === eventKey && isRecentRecord(record, dedupeWindowMs));
  const now = new Date().toISOString();
  ...
  await appendSheetRowRaw(sheets, SHEET_NAME, [
    ...
    normalizeTimestampMinute(eventTimestamp),
    bodyHash,
    duplicated ? 'duplicate_ignored' : 'received',
    duplicated ? 'duplicate_event_key_recent' : 'accepted',
    now,
  ]);
