...
export async function registerWebhookEventAttempt({
  ...
  const resolvedBodyHash = String(bodyHash || '').trim()
  ...
  return { duplicated, eventKey, bodyHash: resolvedBodyHash };
}
...
export async function trackWebhookEventResult({
  ...
  const reasonSafe = [String(reason || '').trim(), String(errorCode || '').trim(), String(traceId || '').trim()]
    .filter(Boolean)
    .join(' | ')
    .slice(0, 240);
  const sheets = createSheetsClient({ readOnly: false });
