...
import { getMexicoDateKey } from './mexicoDate.js';
...
function normalizeDate(value) {
  return getMexicoDateKey(value) || String(value || '').trim();
}
...
async function fetchOrderByIdWithRetry({ storeId, accessToken, orderId }) {
  const requestUrl = `https://api.tiendanube.com/v1/${encodeURIComponent(String(storeId || '').trim())}/orders/${encodeURIComponent(String(orderId || '').trim())}`;
  let lastError = null;
  const totalAttempts = MAX_RETRIES + 1;
  for (let attempt = 1; attempt <= totalAttempts; attempt += 1) {
    try {
      console.log('[ventas:detalle:fetch:start]', {
        order_id: String(orderId || '').trim(),
        store_id: String(storeId || '').trim(),
        token_exists: Boolean(String(accessToken || '').trim()),
...
export async function buildVentasDetalleExport({ ventas = [] } = {}) {
  ...
  const accessToken = normalizeAccessToken(connection?.accessToken || '');
  if (!storeId) {
    throw new Error('Falta configuración de Tiendanube: store_id vacío.');
  }
  if (!accessToken) {
    throw new Error('Falta configuración de Tiendanube: access_token vacío.');
  }
  if (!/^\d+$/.test(storeId)) {
    throw new Error(`store_id inválido para Tiendanube: "${storeId}".`);
  }
  ...
  orderIds.forEach((orderId) => {
    ...
    const baseRow = [
      normalizeDate(venta.fecha_operativa || venta.created_at),
    ...
    if (!products.length) {
      rows.push([
        ...
        String(resolved.httpStatus || 200),
      ]);
      return;
    }
  ...
  const safeRows = rows.map((row, index) => {
    ...
    console.warn('[ventas:detalle:row-length-mismatch]', {
      row_index: index,
