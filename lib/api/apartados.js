...
function getApartadosPdfWebAppUrl() {
  return String(process.env.HARUJA_APARTADOS_PDF_WEBAPP_URL || "").trim();
}

async function requestOfficialPdfFromWebApp(folio) {
  const webAppUrl = getApartadosPdfWebAppUrl();
  if (!webAppUrl) {
    return {
      status: 500,
      body: {
        ok: false,
        message: "Falta configurar HARUJA_APARTADOS_PDF_WEBAPP_URL para generar el PDF oficial.",
      },
    };
  }
  ...
  const response = await fetch(webAppUrl, {
    ...
    body: JSON.stringify({ action: "generar_pdf_apartado", folio }),
  });
  ...
  try {
    ...
    return {
      ...
      body: {
        ok: false,
        message: `Apps Script devolvió una respuesta no JSON: ${rawBody || "vacía"}`,
      },
    };
  }
  ...
  if (!response.ok || !payload?.ok) {
    return {
      ...
      body: {
        ok: false,
        message: String(payload?.message || payload?.error || `HTTP ${response.status}`).trim(),
...
async function updateApartadoPdfMetadata({ folio, pdfUrl, fileId, updatedAt }) {
  ...
  return { pdfUrl: cleanedPdfUrl, hasOfficialPdf: hasOfficialPdf === "true" };
}
...
export async function regenerateApartadoPdf(folio, payload = {}) {
  ...
  const webAppResponse = await requestOfficialPdfFromWebApp(normalizedFolio);
  if (webAppResponse?.status && webAppResponse.status !== 200) {
    return webAppResponse;
  }

  const result = webAppResponse?.body || {};
  const pdfUrl = String(result?.pdfUrl || "").trim();
  if (!pdfUrl) {
    return {
      status: 502,
      body: {
        ok: false,
        message: String(result?.message || "No se recibió pdfUrl real desde Apps Script.").trim(),
  ...
  await updateApartadoPdfMetadata({ folio: normalizedFolio, pdfUrl, fileId, updatedAt });

  return {
    ok: true,
    folio: String(result?.folio || normalizedFolio).trim(),
    pdfUrl,
    fileId,
    folderId: String(result?.folderId || result?.driveFolderId || "").trim(),
    updatedAt,
