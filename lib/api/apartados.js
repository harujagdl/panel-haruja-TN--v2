...
async function updateApartadoPdfMetadata({ folio, pdfUrl, fileId, updatedAt, hasOfficialPdf }) {
  ...
  const shouldMarkOfficial = typeof hasOfficialPdf === "boolean" ? hasOfficialPdf : Boolean(cleanedPdfUrl);
...
export async function regenerateApartadoPdf(folio, payload = {}) {
  ...
  const appUrl = String(payload?.appUrl || process.env.APP_URL || "").trim().replace(/\/$/, "");
  const pdfUrl = appUrl
    ? `${appUrl}/api/pdf-apartado?folio=${encodeURIComponent(normalizedFolio)}`
    : `/api/pdf-apartado?folio=${encodeURIComponent(normalizedFolio)}`;
  const updatedAt = nowIso();
  await updateApartadoPdfMetadata({ folio: normalizedFolio, pdfUrl: "", fileId: "", updatedAt, hasOfficialPdf: true });
  ...
  return {
    ...
    folderId: "",
    updatedAt,
    message: "El PDF oficial ahora se genera desde /api/pdf-apartado.",
    hasOfficialPdf: true,
  };
}
