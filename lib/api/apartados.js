...
export async function regenerateApartadoPdf(folio, payload = {}) {
  ...
  if (!result?.ok) {
    ...
    return { status: 502, body: { ok: false, error: "No se pudo guardar el PDF en Drive", details: result?.details || result?.error || "No se pudo actualizar el PDF.", folio } };
  }
  if (result?.pdfUrl) {
    await updateApartadoPdfMetadata({ folio, pdfUrl: result.pdfUrl, fileId: result.fileId, updatedAt: result.updatedAt });
  }
  return { ok: true, folio, exists: Boolean(result?.exists), pdfUrl: result?.pdfUrl || "", updatedAt: result?.updatedAt || "", fileId: result?.fileId || "", fileName: result?.fileName || "", folderId: result?.folderId || "", driveId: result?.driveId || "", replaced: Boolean(result?.replaced), hasOfficialPdf: Boolean(result?.pdfUrl) };
}
