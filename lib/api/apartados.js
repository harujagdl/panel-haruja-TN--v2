...
export function buildApartadoResponse(apartado, itemsRows, abonosRows) {
  ...
  const saldoPendiente = saldoFuente >= 0 ? saldoFuente : saldoCalculado;
...
function mapApartadoListRow(row) {
  ...
  const saldoCalculado = roundMoney(Math.max(0, total - anticipo));
  const saldo = saldoFuente >= 0 ? saldoFuente : saldoCalculado;
...
export async function addAbono(payload = {}) {
  ...
  logInfo("apartados.abono.parent_updated", { ...logCtx, total, totalAbonadoActual, nuevoTotalAbonado, saldoAnterior, nuevoSaldo, nuevoEstado });
