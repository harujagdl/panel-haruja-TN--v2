...
function isFechaLimiteVencida(fechaLimite, referenceDate = nowIso()) {
  const limiteRaw = String(fechaLimite || "").trim();
  if (!limiteRaw) return false;
  const limite = new Date(limiteRaw);
  const referencia = new Date(referenceDate);
  if (Number.isNaN(limite.getTime()) || Number.isNaN(referencia.getTime())) return false;
  limite.setHours(23, 59, 59, 999);
  referencia.setHours(0, 0, 0, 0);
  return limite.getTime() < referencia.getTime();
}

function resolveApartadoEstado({ saldo = 0, estadoAnterior = "ACTIVO", fechaLimite = "", referenceDate = nowIso() } = {}) {
  const saldoNormalizado = roundMoney(saldo);
  if (saldoNormalizado <= 0) return "LIQUIDADO";

  const estadoPrevio = String(estadoAnterior || "ACTIVO").trim().toUpperCase();
  if (estadoPrevio === "VENCIDO") return "VENCIDO";
  if (isFechaLimiteVencida(fechaLimite, referenceDate)) return "VENCIDO";
  return "ACTIVO";
}
function buildApartadoResumen(apartado = {}, totalAbonado = 0) {
  ...
  const estado = resolveApartadoEstado({
    saldo,
    estadoAnterior: apartado?.Estado || "ACTIVO",
    fechaLimite: apartado?.FechaLimite,
  });
  return { subtotal, anticipo, descVal, total, saldo, estado };
}

function mapHistorialAbonos(abonosRows = [], folioKey = "") {
  return abonosRows
    ...
    .map((abono) => ({
      ...
      tipo: String(abono.Tipo || "ABONO").trim().toUpperCase() || "ABONO",
      fechaCreacion: String(abono.FechaCreacion || "").trim(),
    }));
}

function calcularResumenApartado(apartado = {}, items = [], historialAbonos = []) {
  const subtotalProductos = roundMoney(items.reduce((sum, item) => {
    const cantidad = Number(item?.cantidad ?? item?.Cantidad ?? 1) || 1;
    const precio = roundMoney(item?.precio ?? item?.Precio ?? 0);
    return sum + (precio * cantidad);
  }, 0));
  const subtotalBase = subtotalProductos > 0 ? subtotalProductos : roundMoney(apartado?.Subtotal ?? apartado?.subtotal);
  const descuento = roundMoney(apartado?.DescuentoMXN ?? apartado?.descuento ?? 0);
  const totalFinal = roundMoney(Math.max(0, subtotalBase - descuento));

  let pagos = [...historialAbonos];
  const anticipoCampo = roundMoney(apartado?.Anticipo ?? apartado?.anticipo ?? 0);
  const hasAnticipoEnHistorial = pagos.some((abono) => String(abono?.tipo || '').toUpperCase() === 'ANTICIPO');
  if (!hasAnticipoEnHistorial && anticipoCampo > 0) {
    pagos.push({
  ...
  pagos = pagos
    .filter((p) => roundMoney(p?.monto) > 0)
    .sort((a, b) => Date.parse(String(a?.fecha || '')) - Date.parse(String(b?.fecha || '')));

  const totalAbonado = roundMoney(pagos.reduce((acc, p) => acc + roundMoney(p?.monto), 0));
  const saldoPendiente = roundMoney(Math.max(totalFinal - totalAbonado, 0));
  const sobrepago = roundMoney(Math.max(totalAbonado - totalFinal, 0));
  const estado = resolveApartadoEstado({
  ...
  return { subtotalProductos: subtotalBase, descuento, totalFinal, totalAbonado, saldoPendiente, sobrepago, estado, pagos };
}
...
export function buildApartadoResponse(apartado, itemsRows, abonosRows) {
  ...
  const historialAbonos = mapHistorialAbonos(abonosRows, folioKey);
  const resumen = calcularResumenApartado(apartado, productos, historialAbonos);
  ...
  return {
    ...
    productos,
    subtotal: resumen.subtotalProductos,
    descuento: resumen.descuento,
    anticipo: resumen.totalAbonado,
    total: resumen.totalFinal,
    totalAbonado: resumen.totalAbonado,
    saldoPendiente: resumen.saldoPendiente,
    sobrepago: resumen.sobrepago,
    historialAbonos: resumen.pagos,
...
function mapApartadoListRow(row) {
  ...
  const estatus = resolveApartadoEstado({
    saldo,
    estadoAnterior: row?.EstadoReconciliado || row?.estadoReconciliado || getStatusValue(row),
...
export async function listApartados(params = {}) {
  ...
  const reconciledRows = apartadosRows.map((row) => {
    ...
    const estadoReal = resolveApartadoEstado({
      saldo: saldoReal,
      estadoAnterior: estadoPadre,
      fechaLimite: row?.FechaLimite,
    });
...
export async function addAbono(payload = {}) {
  ...
  logInfo("apartados.abono.start", { ...logCtx, montoAbono, result: "started" });

  if (!folio || !operationId || montoAbono <= 0) {
    logWarn("apartados.abono.invalid_payload", { ...logCtx, errorCode: "INVALID_PAYLOAD" });
    logError("apartados.abono.failed", { ...logCtx, montoAbono, partial: false, code: "INVALID_PAYLOAD", durationMs: Date.now() - startedAt });
  ...
  const [apartadosHeaders, abonosHeaders, apartadosRows, abonosRows, itemsRows] = await Promise.all([
    getSheetHeaders(sheets, spreadsheetId, "apartados"),
  ...
  const duplicateByPayload = abonosRows.find((row) => (
    normalizeFolioKey(row.Folio) === normalizeFolioKey(folio)
    && roundMoney(row.Monto) === montoAbono
    && String(row.Fecha || '').trim() === fecha
    && String(row.Tipo || 'ABONO').trim().toUpperCase() === 'ABONO'
    && String(row.Comentario || '').trim() === (referencia || 'Abono registrado desde formulario')
  ));
  if (!claim.won) {
    ...
    if (duplicatedAbono) {
      ...
      logInfo("apartados.abono.duplicated", { ...logCtx, montoAbono, partial: false, code: "ABONO_DUPLICATED", durationMs: Date.now() - startedAt });
      return {
        ok: true,
  ...
  if (existingAbono || duplicateByPayload) {
    const duplicated = existingAbono || duplicateByPayload;
    const folioRegistrado = normalize(duplicated.Folio) || folio;
    const apartadoExistente = apartadosRows.find((row) => normalize(row.Folio) === folioRegistrado);
    const folioRegistradoKey = normalizeFolioKey(folioRegistrado);
    const abonosFolioReal = abonosRows.filter((row) => normalizeFolioKey(row.Folio) === folioRegistradoKey);
    const totalAbonadoHistorico = sumAbonos(abonosFolioReal);
  ...
  if (!apartado) {
    ...
    logError("apartados.abono.failed", { ...logCtx, montoAbono, partial: false, code: "APARTADO_NOT_FOUND", durationMs: Date.now() - startedAt });
    throw buildAppError("No se encontró el folio.", "APARTADO_NOT_FOUND");
  }

  const folioKey = normalizeFolioKey(folio);
  const productos = itemsRows
    .filter((row) => normalizeFolioKey(row.Folio) === folioKey)
    .map((item) => ({
      cantidad: Number(item.Cantidad || 1) || 1,
      precio: roundMoney(item.Precio),
    }));
  const historialActual = mapHistorialAbonos(abonosRows, normalize(folio));
  const resumenActual = calcularResumenApartado(apartado, productos, historialActual);
  const totalAbonadoActual = resumenActual.totalAbonado;
  const saldoAnterior = resumenActual.saldoPendiente;
  if (saldoAnterior <= 0) {
    logError("apartados.abono.failed", { ...logCtx, montoAbono, saldoAnterior, partial: false, code: "ABONO_ALREADY_SETTLED", durationMs: Date.now() - startedAt });
  ...
  if (montoAbono > saldoAnterior) {
    ...
    logError("apartados.abono.failed", { ...logCtx, montoAbono, saldoAnterior, partial: false, code: "ABONO_INCONSISTENT", durationMs: Date.now() - startedAt });
    throw buildAppError("Este abono supera el saldo pendiente. Revisa el monto antes de continuar.", "ABONO_INCONSISTENT");
  }

  const historialConNuevoAbono = [
    ...historialActual,
    {
      fecha,
      monto: montoAbono,
  ...
  const resumenNuevo = calcularResumenApartado(apartado, productos, historialConNuevoAbono);
  const nuevoTotalAbonado = resumenNuevo.totalAbonado;
  const nuevoSaldo = resumenNuevo.saldoPendiente;
  const nuevoEstado = resumenNuevo.estado;
  const estadoResultante = nuevoEstado;
  ...
  try {
    ...
    if (!pdfSync?.ok) {
      pdfError = String(pdfSync?.details || pdfSync?.error || "").trim() || "No se pudo sincronizar el PDF oficial.";
    }
    if (pdfSync?.ok) logInfo("apartados.abono.pdf_success", logCtx);
    else logWarn("apartados.abono.pdf_failed", { ...logCtx, result: "partial", message: pdfError });
  } catch (error) {
  ...
  logInfo(pdfError ? "apartados.abono.partial" : "apartados.abono.success", {
    ...logCtx,
    folio,
    montoAbono,
    saldoAnterior,
    ...
    code: pdfError ? "PDF_STALE" : "OK",
    durationMs: Date.now() - startedAt,
    result: pdfError ? "partial" : "success",
  });
  ...
  return {
    ...
    duplicated: false,
    partial: Boolean(pdfError),
    ...(pdfError ? { code: "PDF_STALE", pdfStale: true } : {}),
    resumen,
    message: pdfError
      ? "Abono registrado correctamente, pero el PDF oficial quedó desactualizado temporalmente."
...
export async function createApartado(payload = {}) {
  ...
  const historialAbonos = anticipo > 0
    ? [{
      fecha,
      monto: anticipo,
      tipo: "ANTICIPO",
  ...
  const pdfSync = await syncApartadoPdf({
    folio,
    reason: "create",
    apartado: { traceId, folio, cliente, contacto, fecha, subtotal, anticipo, descuento: descuentoMXN, total, saldoPendiente: saldo, status: estado },
