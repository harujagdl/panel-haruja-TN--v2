...
export async function regenerateApartadoPdf(folio, payload = {}) {
  ...
  return { ok: true, folio, exists: Boolean(result?.exists), pdfUrl: result?.pdfUrl || "", updatedAt: result?.updatedAt || "", fileId: result?.fileId || "", fileName: result?.fileName || "", replaced: Boolean(result?.replaced), hasOfficialPdf: Boolean(result?.pdfUrl) };
}
