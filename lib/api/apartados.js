...
export function buildApartadoResponse(apartado, itemsRows, abonosRows) {
  ...
  const anticipo = roundMoney(Math.max(anticipoHistorico, roundMoney(apartado.Anticipo)));
  const saldoFuente = roundMoney(apartado?.Saldo);
  const saldoCalculado = roundMoney(Math.max(0, total - anticipo));
  const saldoPendiente = saldoFuente >= 0 ? roundMoney(Math.min(saldoFuente, saldoCalculado)) : saldoCalculado;
...
function mapApartadoListRow(row) {
  ...
  const saldoFuente = roundMoney(row?.Saldo);
  const saldoCalculado = roundMoney(Math.max(0, total - anticipo));
  const saldo = saldoFuente >= 0 ? roundMoney(Math.min(saldoFuente, saldoCalculado)) : saldoCalculado;
  const estatusFuente = getStatusValue(row);
...
export async function listApartados(params = {}) {
  ...
  const { sheets, spreadsheetId } = await getContext();
  const abonosRows = await readSheetRows(sheets, spreadsheetId, "apartados_abonos");
  const abonosByFolio = new Map();
  abonosRows.forEach((row) => {
    const key = normalize(row?.Folio);
    if (!key) return;
  ...
  const reconciledRows = apartadosRows.map((row) => {
    const folioKey = normalize(row?.Folio);
    const total = roundMoney(row?.Total);
    const anticipoHistorico = sumAbonos(abonosByFolio.get(folioKey) || []);
    const anticipoPadre = roundMoney(row?.Anticipo);
    const anticipoReal = roundMoney(Math.max(anticipoPadre, anticipoHistorico));
    const saldoPadre = roundMoney(row?.Saldo);
    const saldoCalculado = roundMoney(Math.max(0, total - anticipoReal));
    const saldoReal = saldoPadre >= 0 ? roundMoney(Math.min(saldoPadre, saldoCalculado)) : saldoCalculado;
    const estadoPadre = getStatusValue(row);
    const estadoReal = saldoReal <= 0 ? "LIQUIDADO" : estadoPadre;
    return { ...row, Anticipo: anticipoReal, Saldo: saldoReal, Estado: estadoReal };
  });
...
export async function addAbono(payload = {}) {
  ...
  const totalAbonadoActual = roundMoney(Math.max(roundMoney(apartado.Anticipo), totalAbonadoHistorico));
  ...
  const saldoCalculado = roundMoney(total - nuevoTotalAbonado);
  const nuevoSaldo = saldoCalculado <= 0 ? 0 : saldoCalculado;
