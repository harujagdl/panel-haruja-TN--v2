...
export async function resolveTiendanubeConnection() {
  ...
  const storeId = envStoreId || configStoreId || FIXED_STORE_ID;

  const envToken = String(process.env.TIENDANUBE_ACCESS_TOKEN || '').trim();
  const configToken = String(config?.access_token || '').trim();
  const accessToken = envToken || configToken;
  const tokenSource = envToken ? 'TIENDANUBE_ACCESS_TOKEN' : (configToken ? 'VentasConfig.access_token' : 'none');
  const storeSource = envStoreId ? 'TIENDANUBE_STORE_ID' : (configStoreId ? 'VentasConfig.store_id' : 'FIXED_STORE_ID');
...
export { listPrendas, createPrenda, deletePrenda, archivePrenda, generarCodigoPrenda, listArchivedPrendas, restorePrenda, importCorrections };
...
export async function syncVentasFromTiendanubeIncremental(options = {}) {
  ...
  const storeId = connection.storeId;
  const defaultRate = normalizeCommissionRate(config?.default_commission_rate, 0.1);
  const rawOrders = await fetchTiendanubeOrders(storeId, connection.accessToken || '', options);
  const normalizedOrders = rawOrders
    .map((order) => normalizeTiendanubeOrder(order, defaultRate))
    .filter(Boolean);
