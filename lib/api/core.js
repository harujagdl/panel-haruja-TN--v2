...
import { getMexicoDateKey, getMexicoMonthKey } from '../ventas/mexicoDate.js';
...
function currentMonthKey() {
  return getMexicoMonthKey(new Date()) || String(new Date().toISOString()).slice(0, 7);
}
...
function normalizeVentasRows(rows = [], colMap = {}) {
  return rows
    .map((row) => {
      ...
      const monthKeyRaw = colMap.month_key === undefined ? '' : String(row[colMap.month_key] || '').trim();
      const monthKey = /^\d{4}-\d{2}$/.test(monthKeyRaw) ? monthKeyRaw : (getMexicoMonthKey(fecha) || '');
    ...
    .filter((item) => item.monthKey && Number.isFinite(item.total));
}
...
function groupVentasByMonth(ventas = [], year, { onlyPaid = true } = {}) {
  ...
  ventas.forEach((venta) => {
    const [ventaYearRaw, ventaMonthRaw] = String(venta.monthKey || '').split('-');
    const ventaYear = Number(ventaYearRaw);
    const ventaMonth = Number(ventaMonthRaw);
    if (!Number.isFinite(ventaYear) || !Number.isFinite(ventaMonth)) return;
    if (ventaYear !== Number(year)) return;
    if (onlyPaid && venta.paymentStatus && venta.paymentStatus !== 'paid') return;

    const idx = ventaMonth - 1;
...
function mapVentaTN(record = {}) {
  ...
  const monthKeyRaw = pickRecordValue(record, ['month_key']);
  const persistedMonthKey = /^\d{4}-\d{2}$/.test(monthKeyRaw) ? monthKeyRaw : '';
  ...
  return {
    ...
    month_key: persistedMonthKey || getMonthFromDate(fechaOperativa || createdAt),
...
async function ensureVentasTNHeaders(table) {
  ...
  const requiredHeaders = ['ticket_number', 'ticket_label', 'order_id', 'fecha_operativa', 'month_key'];
...
function normalizeTiendanubeOrder(order = {}, defaultRate = 0.1) {
  ...
  return {
    ...
    created_at: createdAt,
    fecha_operativa: fechaOperativa,
    month_key: getMonthFromDate(fechaOperativa || createdAt),
...
export async function repairVentasMonthKeys(context = null) {
  const table = context || await readSheetTable('VentasTN', { readOnly: false });
  const { sheets, headers, rows } = table;
  if (!headers.length || !rows.length) return { scanned: 0, repaired: 0 };
  ...
  if (idxMonth < 0 || idxFechaOperativa < 0) return { scanned: rows.length, repaired: 0 };

  let repaired = 0;
  for (let i = 0; i < rows.length; i += 1) {
    const row = rows[i] || [];
    const venta = mapVentaTN(rowToObject(headers, row));
    const fixedFechaOperativa = getMexicoDateKey(venta.fecha_operativa || venta.created_at);
    const fixedMonthKey = getMonthFromDate(fixedFechaOperativa || venta.created_at);
    const currentFechaOperativa = String(row[idxFechaOperativa] || '').trim();
    const currentMonthKey = String(row[idxMonth] || '').trim();
    if (currentFechaOperativa === fixedFechaOperativa && currentMonthKey === fixedMonthKey) continue;
    const patched = {
      ...venta,
      fecha_operativa: fixedFechaOperativa,
      month_key: fixedMonthKey,
    };
    const rowValues = mapHeadersToRow(headers, patched);
    await updateSheetRowRaw(sheets, `VentasTN!A${i + 2}:ZZ${i + 2}`, rowValues);
    rows[i] = rowValues;
    repaired += 1;
  }
...
export async function syncVentasFromTiendanube() {
  ...
  const { sheets, headers, rows } = table;
  const { orderIdx } = await ensureVentasTNHeaders(table);
  const repairResult = await repairVentasMonthKeys(table);
