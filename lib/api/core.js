...
function readMetasByMonth(rows = [], colMap = {}, year) {
  ...
  rows.forEach((row) => {
    const rowYear = Number(row[colMap.ano] ?? row[colMap['año']] ?? row[colMap.year] ?? 0);
    const rowMonth = Number(row[colMap.mes] ?? row[colMap.month] ?? 0);
    const rowMeta = parseNumber(row[colMap.meta] ?? 0, 0);
...
export async function getMetaVsVentaData(params = {}) {
  ...
  const metasTable = await readSheetTable('Metas', { requiredHeaders: ['mes', 'meta'] });
  const metasByMonth = readMetasByMonth(metasTable.rows || [], getColumnMap(metasTable.headers || []), year);

  return {
    year,
    fromMonth,
    toMonth,
    rows: buildMetaVsVentaRows({ ventasByMonth, metasByMonth, fromMonth, toMonth }),
  };
}
