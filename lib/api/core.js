...
export async function fetchTiendanubeOrders(storeId, providedAccessToken = '', options = {}) {
  ...
  const max429Retries = Math.max(0, parseInt(process.env.TIENDANUBE_SYNC_RETRY_429 || '2', 10) || 2);
  for (let page = 1; page <= maxPages; page += 1) {
    ...
    console.log('[ventas.tiendanube.request]', {
    ...
    while (attempt <= max429Retries) {
      ...
      console.warn('[ventas.tiendanube.retry_429]', { page, attempt: attempt + 1, waitMs, store_id: normalizedStoreId });
      // eslint-disable-next-line no-await-in-loop
      await new Promise((resolve) => setTimeout(resolve, waitMs));
      attempt += 1;
    }

    console.log('[ventas.tiendanube.response]', {
      status: response.status,
      ok: response.ok,
      page,
      traceId: options.traceId || '',
      store_id: normalizedStoreId,
      orders_count: Array.isArray(payload) ? payload.length : 0,
      from: createdAtMin || '',
      to: updatedAtMin || '',
    });
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        throw buildTiendanubeError({
          ...
          status: response.status,
          message: `Tiendanube respondió ${response.status}. Revisar access token, store/user id y scope read_orders. (${detail})`,
