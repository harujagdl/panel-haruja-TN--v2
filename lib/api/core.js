...
export async function getVentasConfig() {
  ...
  for (const row of rows) {
    ...
    if (mapped.store_id || mapped.user_id || mapped.access_token) {
      return {
        ...mapped,
        client_id: mapped.client_id || String(process.env.TIENDANUBE_CLIENT_ID || process.env.TIENDANUBE_APP_ID || '').trim(),
        app_id: mapped.app_id || String(process.env.TIENDANUBE_APP_ID || process.env.TIENDANUBE_CLIENT_ID || '').trim(),
        store_id: mapped.store_id || mapped.user_id || '',
      };
    }
  }
  return null;
