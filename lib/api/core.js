...
function mapVentaTN(record = {}) {
  ...
  return {
    ...
    payment_status: pickRecordValue(record, ['payment_status']),
    fulfillment_status: pickRecordValue(record, ['fulfillment_status']),
    ...
    raw_status: pickRecordValue(record, ['raw_status']),
...
function normalizeTiendanubeOrder(order = {}, defaultRate = 0.1) {
  ...
  const commissionAmount = commissionableTotal * commissionRate;
  ...
  return {
    ...
    subtotal,
    discount,
    payment_status: String(order.payment_status || order.gateway || order.payment_details || '').trim(),
    fulfillment_status: String(order.fulfillment_status || order.shipping_status || '').trim(),
    channel: 'tiendanube',
    seller: '',
    ...
    raw_status: String(order.status || order.payment_status || '').trim(),
    last_sync_at: new Date().toISOString(),
  };
}

export async function fetchTiendanubeOrders(storeId) {
...
export async function getVentasDetalle(monthValue) {
  const monthKey = normalizeMonth(monthValue) || currentMonthKey();
  const { headers, rows } = await readSheetTable('VentasTN');
  if (!headers.length || !rows.length) return [];
...
export async function rebuildVentasResumen(monthValue) {
  ...
  const { headers: ventasHeaders, rows: ventasRows } = await readSheetTable('VentasTN');
  const ventasDelMes = (ventasRows || [])
  ...
  const summary = {
    ...
    sin_asignar: 0,
    comision_total: 0,
    orders_count: ventasDelMes.length,
    updated_at: now,
  };
  ...
  ventasDelMes.forEach((venta) => {
    ...
    const assigned = Boolean(seller) && parseBoolean(venta.seller_assigned, false);

    if (seller === 'Haru') summary.total_haru += total;
    if (seller === 'Vendedora') summary.total_vendedora += total;
    if (!assigned) summary.sin_asignar += 1;
  });

  const resumenTable = await readSheetTable('VentasResumen', { readOnly: false });
...
export async function syncVentasFromTiendanube() {
  ...
  const rawOrders = await fetchTiendanubeOrders(storeId);
