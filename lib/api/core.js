...
export async function syncVentasFromTiendanubeIncremental(options = {}) {
  ...
  const defaultRate = normalizeCommissionRate(config?.default_commission_rate, 0.1);
  const rawOrders = await fetchTiendanubeOrders(storeId, config?.access_token || '', options);
  const normalizedOrders = rawOrders
    .map((order) => normalizeTiendanubeOrder(order, defaultRate))
    .filter(Boolean);
  ...
  normalizedOrders.forEach((venta) => {
    byOrderId.set(venta.order_id, venta);
  });

  let inserted = 0;
  let updated = 0;
  for (const venta of byOrderId.values()) {
    const result = await upsertVentaTN(venta, table);
    if (result?.action === 'inserted') inserted += 1;
    if (result?.action === 'updated') updated += 1;
    if (result?.venta?.month_key) monthsTouched.add(result.venta.month_key);
  }
