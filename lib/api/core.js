...
function mapVentasConfig(record = {}) {
  const userId = pickRecordValue(record, ['user_id', 'userid']);
  const storeId = pickRecordValue(record, ['store_id', 'storeid']) || userId;
  return {
    store_id: storeId,
    user_id: userId || storeId,
    store_name: pickRecordValue(record, ['store_name', 'storename']),
    app_id: pickRecordValue(record, ['app_id', 'appid', 'client_id', 'clientid']),
    client_id: pickRecordValue(record, ['client_id', 'clientid', 'app_id', 'appid']),
...
async function getSharedVentasByMonth(year, { onlyPaid = true } = {}) {
  const ventasDetalle = await getVentasDetalle();
  const ventas = (ventasDetalle || []).map((venta) => ({
    fecha: toDateSafe(venta.created_at),
    month_key: String(venta.month_key || venta.monthKey || getMonthFromDate(venta.created_at) || '').trim(),
    total: parseNumber(venta.total_pagado ?? venta.total_paid ?? venta.total ?? venta.venta, 0),
  ...
  return groupVentasByMonth(ventas, year, { onlyPaid });
}
...
export function tiendanubeApiHeaders(accessToken) {
...
export async function saveVentasConfig(payload = {}) {
  ...
  const userId = String(payload.user_id || storeId).trim() || storeId;
  const appId = String(payload.app_id || payload.client_id || process.env.TIENDANUBE_APP_ID || process.env.TIENDANUBE_CLIENT_ID || '').trim();
  const clientId = String(payload.client_id || payload.app_id || process.env.TIENDANUBE_CLIENT_ID || process.env.TIENDANUBE_APP_ID || '').trim();
  ...
  const rowValuesFromPayload = (createdAtValue, connectedAtValue = '', lastSyncAtValue = '') => headers.map((header) => {
    const normalized = normalizeHeader(header);
    if (normalized === 'store_id') return storeId;
    if (normalized === 'user_id') return userId;
    if (normalized === 'store_name') return storeName;
    if (normalized === 'app_id') return appId;
    if (normalized === 'client_id') return clientId;
    if (normalized === 'access_token') return accessToken;
  ...
  rows.forEach((row, idx) => {
    ...
    if (currentStoreId) {
      ...
      existingConnectedAt = String(row?.[indexByHeader.get('connected_at')] || '').trim();
      existingLastSyncAt = String(row?.[indexByHeader.get('last_sync_at')] || '').trim();
    }
  });

  if (existingRowNumber) {
    const connectedAt = existingConnectedAt || (accessToken ? now : '');
    await updateSheetRowRaw(sheets, `VentasConfig!A${existingRowNumber}:ZZ${existingRowNumber}`, rowValuesFromPayload(existingCreatedAt, connectedAt, existingLastSyncAt));
  } else {
    await appendSheetRowRaw(sheets, 'VentasConfig', rowValuesFromPayload(now, accessToken ? now : '', ''));
  }
  ...
  return {
    ...
    active,
    connected_at: existingConnectedAt || (accessToken ? now : null),
    last_sync_at: existingLastSyncAt || null,
...
export async function saveTiendanubeOAuthConfig(payload = {}) {
  ...
  const tokenValidated = payload.token_validated === true;
  if (!tokenValidated) throw new Error('No se puede guardar OAuth sin validar token contra Tiendanube.');

  const appId = String(payload.app_id || payload.client_id || process.env.TIENDANUBE_APP_ID || process.env.TIENDANUBE_CLIENT_ID || '').trim();
  if (!appId) throw new Error('Falta TIENDANUBE_APP_ID para guardar OAuth.');

  const incomingStoreId = String(payload.store_id || payload.user_id || '').trim();
  if (!incomingStoreId) throw new Error('No se recibió store_id/user_id de Tiendanube.');

  const existingConfig = await getVentasConfig();
  const storeId = incomingStoreId;
  ...
  const buildRow = (createdAtValue, lastSyncAtValue = '') => headers.map((header) => {
    ...
    if (key === 'user_id') return storeId;
    if (key === 'store_name') return storeName;
    if (key === 'app_id') return appId;
    if (key === 'client_id') return appId;
    if (key === 'access_token') return accessToken;
    if (key === 'scope') return String(payload.scope || existingConfig?.scope || '').trim();
  ...
  if (!existingRowNumber) {
    rows.forEach((row, idx) => {
      const currentUserId = String(row?.[indexByHeader.get('user_id')] || '').trim();
      if (currentUserId !== storeId) return;
      existingRowNumber = idx + 2;
      existingCreatedAt = String(row?.[indexByHeader.get('created_at')] || '').trim();
      existingLastSyncAt = String(row?.[indexByHeader.get('last_sync_at')] || '').trim();
    });
  ...
  console.log('[tiendanube.oauth.saved_config]', {
    store_id: storeId,
    user_id: storeId,
  ...
  return {
    ...
    scope: String(payload.scope || existingConfig?.scope || '').trim(),
    default_commission_rate: defaultRate,
    active,
    connected_at: now,
    created_at: existingCreatedAt || now,
...
export async function getVentasResumen(monthValue) {
  ...
  const grouped = await getSharedVentasByMonth(year, { onlyPaid: true });
  const groupedMonth = grouped[month] || {};
...
export async function getMetaVsVentaData(params = {}) {
  ...
  console.log("[meta.vs.venta.start]", {
    params,
    year,
    view,
    fromMonth,
  ...
  console.log("[meta.vs.venta.ventasByMonth]", {
    year,
    ventasByMonth
  });
  ...
  console.log("[meta.vs.venta.metasByMonth]", {
    year,
    metasByMonth
  });
  ...
  console.log('[meta.vs.venta.result]', {
    year,
    fromMonth,
    toMonth,
    rows: rows.map((r) => ({
...
export { syncVentasFromTiendanube } from '../ventas/tiendanubeSync.js';
