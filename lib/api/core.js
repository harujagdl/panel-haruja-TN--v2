...
export async function fetchTiendanubeOrders(storeId, providedAccessToken = '', options = {}) {
  ...
  const logCtx = { traceId, storeId: normalizedStoreId, updatedAtMin: updatedAtMin || null, createdAtMin: createdAtMin || null };
  for (let page = 1; page <= maxPages; page += 1) {
    ...
    logInfo('ventas.tiendanube.request', { ...logCtx, page, url: url.toString() });
    ...
    if (response.status === 429) {
      await new Promise((resolve) => setTimeout(resolve, 600 * page));
    ...
    logInfo('ventas.tiendanube.response', {
      ...logCtx,
      page,
      status: response.status,
      ok: response.ok,
      ordersReceived: Array.isArray(payload) ? payload.length : 0,
    });
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        throw buildTiendanubeError({
          ...
          status: response.status,
          message: `Tiendanube respondió ${response.status}. Revisar access token, store/user id y scope read_orders. Detalle: ${detail}`,
