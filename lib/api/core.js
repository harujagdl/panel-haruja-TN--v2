...
function isPaidVenta(venta = {}) {
  const paymentStatus = normalizeStatusToken(venta.payment_status);
  if (paymentStatus === 'paid') return true;

  const [combinedPayment] = splitCombinedStatus(paymentStatus);
  if (combinedPayment === 'paid') return true;

  const [rawPayment] = splitCombinedStatus(venta.raw_status);
  if (rawPayment === 'paid') return true;
...
export async function rebuildVentasResumen(monthValue) {
  ...
  const paidVentasDelMes = ventasDelMes.filter((venta) => isPaidVenta(venta));
  ...
  const summary = {
    ...
    sin_asignar: 0,
    comision_total: 0,
    orders_count: paidVentasDelMes.length,
    updated_at: now,
  };

  paidVentasDelMes.forEach((venta) => {
