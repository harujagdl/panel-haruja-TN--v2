...
import { createTraceId, logInfo, logWarn } from "../observability/logger.js";
...
export async function updatePrenda(payload = {}) {
  ...
  const codigo = String(payload?.codigo || "").trim();
  if (!codigo) {
    const err = new Error("El campo 'codigo' es obligatorio.");
    err.code = "INVALID_PAYLOAD";
    throw err;
  }
  logInfo("prendas.update.start", { action: "prendas-update", op: "update", codigo, traceId });
  const payloadKeys = Object.keys(payload || {});
  const unknownKeys = payloadKeys.filter((key) => !UPDATE_ALLOWED_PAYLOAD_KEYS.has(String(key || "").trim()));
  if (unknownKeys.length) {
    logWarn("prendas.update.invalid_payload", {
      action: "prendas-update",
      op: "update",
      codigo,
      traceId,
      errorCode: "INVALID_PAYLOAD",
      ignoredKeys: unknownKeys,
    });
  }
  ...
  if (!hasPrecio && !hasCosto && !hasStatus && !hasDisponibilidad && !hasExistencia) {
    const err = new Error("No se enviaron campos editables permitidos.");
    err.code = "INVALID_PAYLOAD";
    throw err;
  }
  ...
  if (!matchedRow) {
    logWarn("prendas.update.not_found", {
      action: "prendas-update",
      op: "update",
    ...
    return { status: 404, body: { ok: false, code: "PRENDA_NOT_FOUND", traceId, message: "Código no encontrado en prendas_admin_activas." } };
  }
  ...
  logInfo("prendas.update.success", { action: "prendas-update", op: "update", codigo: normalizedCode, traceId, result: "success" });
  return { ok: true, codigo: normalizedCode, updated: true, traceId };
}
