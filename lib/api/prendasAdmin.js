...
const EDITABLE_UPDATE_COLUMNS = ["Descripción", "Precio", "Costo", "Status", "Disponibilidad", "Existencia", "Existencias", "Utilidad", "Margen"];
...
export async function updatePrenda(payload = {}) {
  ...
  const keyByHeader = {
    ...
    [existenciaHeader]: "existencia",
    Utilidad: "utilidad",
    Margen: "margen",
  };

  const nextRowObject = { ...(matchedRow.sourceRowObject || {}) };
  const precioFromPayload = Object.prototype.hasOwnProperty.call(payload, "precio")
    ? normalizeNumber(payload.precio)
    : null;
  const costoFromPayload = Object.prototype.hasOwnProperty.call(payload, "costo")
    ? normalizeNumber(payload.costo)
    : null;
  const precioFinal = precioFromPayload ?? normalizeNumber(nextRowObject.Precio) ?? 0;
  const costoFinal = costoFromPayload ?? normalizeNumber(nextRowObject.Costo) ?? 0;
  const { utilidad, margen } = calcularUtilidadYMargen(precioFinal, costoFinal);
  payload.utilidad = utilidad;
  payload.margen = margen;

  EDITABLE_UPDATE_COLUMNS.forEach((header) => {
    if (!headers.includes(header)) return;
    const payloadKey = keyByHeader[header];
    if (!payloadKey) return;
    if (!Object.prototype.hasOwnProperty.call(payload, payloadKey)) return;
    const incomingValue = payload[payloadKey];
    if (NUMERIC_COLUMNS.has(header)) {
      const parsed = normalizeNumber(incomingValue, { integer: header === "Existencia" });
      nextRowObject[header] = parsed === null ? "" : String(parsed);
      return;
    }
    nextRowObject[header] = normalizeText(incomingValue);
  });
