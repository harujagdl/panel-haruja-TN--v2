...
export async function uploadCatalogoIAImage(payload = {}, context = {}) {
  ...
  if (!sku) {
    throw new Error('Primero captura o selecciona un SKU para nombrar la foto.');
  }
  ...
  const url = String(process.env.CATALOGO_IA_PHOTO_WEBAPP_URL || '').trim();
  const token = String(process.env.CATALOGO_IA_PHOTO_WEBAPP_TOKEN || '').trim();

  if (!url) {
    throw new Error('Falta configurar CATALOGO_IA_PHOTO_WEBAPP_URL.');
  }

  if (!token) {
    throw new Error('Falta configurar CATALOGO_IA_PHOTO_WEBAPP_TOKEN.');
  }
  ...
  if (!response.ok || !data?.ok) {
    throw new Error(
      data?.message || 'No fue posible subir la foto con Apps Script.'
    );
  }

  const photo = {
    fileId: normalize(data?.data?.fileId || ''),
    name: normalize(data?.data?.name || ''),
    webViewLink: normalize(data?.data?.webViewLink || ''),
    webContentLink: normalize(data?.data?.webContentLink || '')
  };

  await appendCatalogoIAFoto(
    {
      ...payload,
      ...photo
    },
