...
const CATALOGO_IA_RESPONSE_SCHEMA = {
  ...
  schema: {
    ...
    properties: {
      ...
      detallesDetectados: { type: 'array', items: { type: 'string' } },
...
const normalizeCatalogoIAResult = (value = {}) => ({
  ...
  categoriaSugerida: normalize(value?.categoriaSugerida),
  colorDetectado: normalize(value?.colorDetectado),
  detallesDetectados: normalizeStringArray(value?.detallesDetectados),
  advertencias: normalizeStringArray(value?.advertencias)
});
...
const buildCatalogoIAPrompt = (payload = {}) => {
  const lines = [
    ...
    '',
    'Objetivo:',
    'Crear ficha de producto para Tiendanube a partir de fotos editadas y datos base.',
    '',
    'REGLAS IMPORTANTES',
