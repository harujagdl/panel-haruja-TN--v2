...
export async function getPdfStatus(folio) {
  ...
  return { ok: true, folio, exists: Boolean(result?.exists), pdfUrl: result?.pdfUrl || "", updatedAt: result?.updatedAt || "", fileId: result?.fileId || "", fileName: result?.fileName || "", folderId: result?.folderId || "", replaced: Boolean(result?.replaced) };
}
