...
export async function getPdfStatus(folio) {
  ...
  if (!result?.ok) {
    ...
    return { status: 502, body: { ok: false, error: "No se pudo guardar el PDF en Drive", details: result?.details || result?.error || "No se pudo consultar el PDF.", folio } };
  }
  return { ok: true, folio, exists: Boolean(result?.exists), pdfUrl: result?.pdfUrl || "", updatedAt: result?.updatedAt || "", fileId: result?.fileId || "", fileName: result?.fileName || "", folderId: result?.folderId || "", driveId: result?.driveId || "", replaced: Boolean(result?.replaced) };
}
