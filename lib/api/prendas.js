...
const PRENDA_COLUMN_INDEX_BY_HEADER = {
  Orden: 0,
  Código: 1,
  Descripción: 2,
  Tipo: 3,
...
const createStableCreateKey = (payload = {}) => {
  ...
  const stableData = {
    idempotencyKey: normalize(payload.idempotencyKey || payload.operationId),
...
async function findPrendaByCodigo(sheets, codigo) {
  const normalizedCode = normalizeValue(codigo);
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    const rowCode = normalizeValue(row?.[CODE_COLUMN_INDEX] || "");
    if (rowCode !== normalizedCode) continue;
    return {
      sheetName,
      rowNumber: index + 2,
      orden: String(row?.[0] ?? "").trim(),
      codigo: rowCode,
    };
  }
  return null;
...
function getByNormalizedKey(row, aliases = []) {
  if (!row || Array.isArray(row)) return "";
  const normalizedMap = new Map(
    Object.keys(row).map((key) => [normalizeHeaderKey(key), row[key]])
  );
...
function getCell(row, headerValue, index) {
...
function parseSheetsSerialDate(value) {
...
function formatFechaDisplay(value) {
  ...
  if (/^\d+(\.\d+)?$/.test(raw)) {
    const parsed = parseSheetsSerialDate(raw);
...
export async function listPrendas() {
  ...
  const response = await sheets.spreadsheets.values.get({ spreadsheetId: getSpreadsheetId(), range: `${sheetName}!A1:ZZ3000` });
  const rows = response?.data?.values || [];
  ...
  return rows.slice(1).map((row) => {
    ...
    const proveedor = getByNormalizedKey(obj, ["Proveedor", "Marca", "Supplier"])
      || getCell(row, "Proveedor", PRENDA_COLUMN_INDEX_BY_HEADER.Proveedor);
    const fechaRaw = getByNormalizedKey(obj, ["Fecha", "Fecha Alta", "Fecha de alta", "Alta"])
      || getCell(row, "Fecha", PRENDA_COLUMN_INDEX_BY_HEADER.Fecha);
    const orden = getByNormalizedKey(obj, ["Orden"])
      || getCell(row, "Orden", PRENDA_COLUMN_INDEX_BY_HEADER.Orden);

    return {
      ...obj,
      Orden: obj.Orden ?? orden,
      Proveedor: proveedor,
      proveedor,
      Fecha: fechaRaw,
      fecha: fechaRaw,
      fechaTexto: formatFechaDisplay(fechaRaw),
    };
  });
}
...
export async function createPrenda(payload = {}) {
  ...
  const idempotencyKey = normalizeValue(payload.idempotencyKey || payload.operationId || "");
  ...
  const createPromise = withCreatePrendaLock(async () => {
    ...
    const duplicateBeforeBuild = await findPrendaByCodigo(sheets, codigoFinal);
    if (duplicateBeforeBuild) {
      return {
        ok: true,
        duplicateRequest: true,
        idempotent: Boolean(idempotencyKey),
        codigo: duplicateBeforeBuild.codigo,
        codigoFinal: duplicateBeforeBuild.codigo,
        orden: duplicateBeforeBuild.orden,
        ordenFinal: duplicateBeforeBuild.orden,
        code: "DUPLICATE_CODIGO",
        message: "El código ya existe; se devolvió el registro existente.",
    ...
    const duplicateBeforeAppend = await findPrendaByCodigo(sheets, codigoFinal);
    if (duplicateBeforeAppend) {
      return {
        ok: true,
        duplicateRequest: true,
        idempotent: Boolean(idempotencyKey),
        codigo: duplicateBeforeAppend.codigo,
        codigoFinal: duplicateBeforeAppend.codigo,
        orden: duplicateBeforeAppend.orden,
        ordenFinal: duplicateBeforeAppend.orden,
        code: "DUPLICATE_CODIGO",
        message: "El código ya existe; se canceló el append para evitar duplicado.",
        columns: ADMIN_COLUMNS,
      };
    }
    ...
    const persisted = await findPrendaByCodigo(sheets, codigoFinal);
    if (!persisted) {
      throw new Error("No se pudo confirmar la persistencia del código después del guardado.");
    }
    const result = {
      ok: true,
      codigo: codigoFinal,
