...
async function buildPrendaDescripcion(sheets, payload = {}) {
  ...
  if (!tipoInput && !colorInput && !tallaInput && !parsedFromCode.tipo && !parsedFromCode.color && !parsedFromCode.talla) {
    return String(payload.descripcion || "").trim().replace(/\s+/g, " ");
  }
  ...
  if (!descripcionBase) return String(payload.descripcion || "").trim().replace(/\s+/g, " ");
  return detallesLimpios ? `${descripcionBase} - ${detallesLimpios}` : descripcionBase;
}
