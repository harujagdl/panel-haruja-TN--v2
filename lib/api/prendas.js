...
const lookupDictionaryValue = (dictionaryMap, key) => {
  const safeKey = normalizeDictionaryKey(key);
  if (!safeKey) return "";
  return dictionaryMap.get(safeKey) || safeKey;
};

async function buildPrendaDescripcion(sheets, payload = {}) {
  const parsedFromCode = parseCodeParts(payload.codigo);
  const tipoClave = normalizeValue(payload.tipo || parsedFromCode.tipo || "");
  const colorClave = normalizeValue(payload.color || parsedFromCode.color || "");
  const tallaClave = normalizeValue(payload.talla || parsedFromCode.talla || "");

  if (!tipoClave && !colorClave && !tallaClave) {
    return String(payload.descripcion || "").trim();
  }
  ...
  const nombreCategoria = lookupDictionaryValue(categoriesByKey, tipoClave);
  const nombreColor = lookupDictionaryValue(colorsByKey, colorClave);
  const nombreTalla = lookupDictionaryValue(sizesByKey, tallaClave);

  const descripcion = [nombreCategoria, nombreColor, nombreTalla].filter(Boolean).join(" ").trim();
  return descripcion || String(payload.descripcion || "").trim();
}
...
export async function generarCodigoPrenda(payload = {}) {
  ...
  const codigosRows = await listPrendas();
  const baseCodigos = codigosRows
    .map((row) => normalizeValue(row?.["Código"] || row?.codigo || ""))
    .filter(Boolean);
