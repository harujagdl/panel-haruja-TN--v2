  ...
  return rows
    .map((row = []) => {
      ...
      const isHeaderNoise =
        ...
        ['valor', 'value'].includes(valor.toLowerCase());
      if (isHeaderNoise) return null;
      const dedupeKey = `${clave}::${valor}`;
...
const loadDictionarySheet = async (sheets, sheetName) => {
  ...
  const parsed = toDictionaryEntries(response?.data?.values || []);
  if (sheetName === DICTIONARY_SHEETS.tallas) {
    console.log(`[tallas] parsed count: ${parsed.length}`);
  }
  return parsed;
};
